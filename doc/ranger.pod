...
C<sqlite3> for listing tables in SQLite database (and optionally C<sqlite-utils> for fancier box drawing.)
...
This does not work over ssh, requires certain terminals (tested on "xterm",
"urxvt", and "Alacritty") and is compatible with tmux and screen.
...
Adding new commands via a plugin is as simple as specifying them like you would do
...
Toggle between viewmodes. See viewmode in I<SETTINGS> section.
...
value are in [brackets].  The hotkey to toggle the setting is in <brackets>, if
a hotkey exists.
...
Each binary unit prefix signifies 1024=2^10 of the preceding unit prefix (there
are 1024 KiB in 1 MiB) as opposed to decimal unit prefixes which signify a 1000
of the preceding unit prefix (there are 1000 kB in 1 MB)
...
=item Download: L<https://ranger.fm/ranger-stable.tar.gz>

=item The project page: L<https://ranger.fm/>
